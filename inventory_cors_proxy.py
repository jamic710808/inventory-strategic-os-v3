...
# -*- coding: utf-8 -*-
"""
Inventory Strategic OS — 一體化本機伺服器
  ① 靜態檔案服務（開啟儀表板 HTML）
  ② 智慧 CORS Proxy（轉發至任意 AI API，目標由 X-Proxy-Target header 決定）

使用方式（只需一個指令）：
  python inventory_cors_proxy.py            # 預設 Port 8765
  python inventory_cors_proxy.py 9000       # 指定 Port

啟動後：
  1. 瀏覽器開啟 http://localhost:8765
  2. 點選 Inventory_Strategic_OS_V4.html
...
class InventoryServerHandler(BaseHTTPRequestHandler):
    ...
    def _dir_listing(self, local_dir, url_path):
        ...
        rows = ''.join(rows)
        html = f'''<!DOCTYPE html><html><head><meta charset="utf-8"><title>Inventory Strategic OS</title>
<style>body{{font-family:sans-serif;padding:24px;background:#0d0d1a;color:#ddd}}
h2{{color:#38bdf8}}a{{color:#7ec8e3;text-decoration:none}}a:hover{{color:#38bdf8}}
table{{border-collapse:collapse;width:100%}}td{{padding:7px 14px;border-bottom:1px solid #222}}
</style></head><body>
<h2>📦 Inventory Strategic OS — 儀表板目錄</h2>
<table>{rows}</table>
<hr style="border-color:#333;margin-top:24px">
<small style="color:#555">Inventory CORS Proxy · Port {PORT} · 端點填原始 URL，開啟面板「CORS 代理」即可</small>
</body></html>'''
        data = html.encode()
        self.send_response(200)
